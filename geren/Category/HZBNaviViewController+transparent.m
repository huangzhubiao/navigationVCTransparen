//
//  HZBNaviViewController+transparent.m
//  geren
//
...
@implementation HZBNaviViewController (transparent)
// 设置导航栏背景透明度
...
- (void)et__updateInteractiveTransition:(CGFloat)percentComplete {
    ...
    if (topVC != nil) {
        ...
        if (coor != nil) {
            // 随着滑动的过程设置导航栏透明度渐变
            CGFloat fromAlpha = [((HZBBaseViewController *)[coor viewControllerForKey:UITransitionContextFromViewControllerKey]).navBarBgAlpha floatValue];
            CGFloat toAlpha = [((HZBBaseViewController *)[coor viewControllerForKey:UITransitionContextToViewControllerKey]).navBarBgAlpha floatValue];
...
- (void)dealInteractionChanges:(id<UIViewControllerTransitionCoordinatorContext>)context {
    if ([context isCancelled]) {// 自动取消了返回手势
        ...
        [UIView animateWithDuration:cancelDuration animations:^{
            CGFloat nowAlpha = [((HZBBaseViewController*)[context viewControllerForKey:UITransitionContextFromViewControllerKey]).navBarBgAlpha floatValue];
        ...
        [UIView animateWithDuration:finishDuration animations:^{
            CGFloat nowAlpha = [((HZBBaseViewController *)[context viewControllerForKey:
...
- (void)navigationBar:(UINavigationBar *)navigationBar didPopItem:(UINavigationItem *)item {
    if (self.viewControllers.count >= navigationBar.items.count) {// 点击返回按钮
        HZBBaseViewController *popToVC = (HZBBaseViewController *)self.viewControllers[self.viewControllers.count - 1];
...
- (void)navigationBar:(UINavigationBar *)navigationBar didPushItem:(UINavigationItem *)item {
    // push到一个新界面
    [self setNeedsNavigationBackground:[((HZBBaseViewController *)self.topViewController).navBarBgAlpha floatValue]];
}
//定义常量 必须是C语言字符串
